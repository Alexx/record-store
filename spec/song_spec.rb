...
describe '#Song' do
  ...
  before(:each) do
    Album.clear()
    Song.clear()
    @album = Album.new("Battle for Los Angeles", nil, "Artist", "Year", "Genre")
    @album.save()
  end

  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      song = Song.new("Fist Full of Steel", @album.id, nil)
      song2 = Song.new("Fist Full of Steel", @album.id, nil)
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new("Guerilla Radio", @album.id, nil)
      song.save()
      song2 = Song.new("Fist Full of Steel", @album.id, nil)
      song2.save()
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      album2 = Album.new("De Stijl", nil, "Artist", "Year", "Genre")
      album2.save
      song = Song.new("Guerilla Radio", @album.id, nil)
      song.save()
      song2 = Song.new("I Can Tell That We Are Gonna Be Friends", album2.id , nil)
      song2.save()
